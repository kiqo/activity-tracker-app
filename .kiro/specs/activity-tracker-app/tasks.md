...
- [ ] 16. Implement shared location tracking with junction table
  - [ ] 16.1 Update database schema for shared location points
    ...
    - Add indexes on sessionId and locationPointId columns
  ...
  - [ ] 16.2 Update LocationRepository for shared location tracking
    ...
    - Update `getLocationPointsForSession()` to query via junction table
...
- [ ] 18. Write unit tests for dual-track and shared location features
  - [ ] 18.1 Test dual-track session management
    ...
    - Test that new manual session stops existing manual session
    - Test that new automatic session stops existing automatic session
  ...
  - [ ] 18.2 Test shared location tracking
    ...
    - Test that LocationTrackingService stops only when all sessions end
    - Test junction table cascade delete behavior
    - _Requirements: 2.1, 2.5, 2.6_
